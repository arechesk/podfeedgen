...
def main(curDir=path.abspath('.')):
	for i in filter(lambda x: path.isdir(x), os.listdir(curDir)):
		for j in filter(lambda x: path.splitext(x)[1] in ['.avi','.mpg','.mkv','.ogv','.wmv','.flv','.ogg'],os.listdir(i)):
			os.system('HandBrakeCLI -i "./'+i+'/'+j+'" -o "./'+i+'/'+path.splitext(j)[0]+'.mov"')
			os.system('rm -f "./'+i+'/'+j+'"')
	listDir = filter(lambda x: path.splitext(x)[1] in ['.FLV','.avi','.mpg','.ogv','.mkv', '.wmv', '.flv','.ogg'], os.listdir(curDir))
